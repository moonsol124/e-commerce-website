...
function ItemUpdateForm(props) {
    ...
    function onSubmit(e) {
        e.preventDefault();
        const url = `http://localhost:3000/admin/color/${props.id}/update`;
        const data = {};
    ...
    return (
        <form className="form-container item-update-form" onSubmit={onSubmit}>
            ...
            <div className="item-form-div">
                ...
                <input type="color" name="rgbValue" id="rgbValue" defaultValue={props.item.rgbValue}></input>
            </div>
            <div>
                <p>{message}</p>
            </div>
...
function Description(props) {
    ...
    function deleteItem() {
        const url = `http://localhost:3000/admin/color/${props.id}/delete`;
        const requestOptions = {
    ...
    return (
        <ul className="items-ul">
            ...
            <li className="li-grid-two-colums"> color: <span style={{backgroundColor: props.rgbValue}} className="color-div"></span></li>
...
function ItemForm(props) {
    ...
    function onSubmit(e) {
        e.preventDefault();
        const url = "http://localhost:3000/admin/color/create";
        const data = {};
        for (let i = 0; i < e.target.length; i++) {
            const fields = e.target[i];
            if (fields.value.length > 0) {
    ...
    return (
        <form className="form-container" onSubmit={onSubmit}>
            ...
            <div className="item-form-div">
                <label htmlFor="rgbValue">Color: </label>
                <input type="color" name="rgbValue" id="rgbValue"></input>
            </div>
            <div>
                <p>{message}</p>
            </div>
...
function Item() {
    ...
    useEffect(() => {
        ...
        .then(response => {
            if (response.status === 200) {
                setItems(response.colors.colors);
            }
        })
    }, [newItemAdded])
    return (
