...
function ItemUpdateForm(props) {
    ...
    function onSubmit(e) {
        e.preventDefault();
        const url = `http://localhost:3000/admin/size/${props.id}/update`;
        const data = {};
...
function Description(props) {
    ...
    function deleteItem() {
        const url = `http://localhost:3000/admin/size/${props.id}/delete`;
        const requestOptions = {
...
function ItemForm(props) {
    ...
    function onSubmit(e) {
        e.preventDefault();
        const url = "http://localhost:3000/admin/size/create";
        const data = {};
        for (let i = 0; i < e.target.length; i++) {
            const fields = e.target[i];
            if (fields.value.length > 0) {
